...
class cis_benchmarks::redhat7::rule::v_2_1_1::rule_2_2_1_2 (
  ...
  String $time_server = lookup("cis_benchmarks::${cis_version}::time_server",String,'first',$cis_benchmarks::params::time_server),
    ...
    file_line { "(2.2.1.2) ${file} - ensure ExecStart=/usr/sbin/ntpd -u ntp:ntp $OPTIONS":
      ensure => present,
      path   => $file,
      line   => 'ExecStart=/usr/sbin/ntpd -u ntp:ntp $OPTIONS',
      require => Package[$time_server]
    }
    file_line { "(2.2.1.2) ${file} - ensure ExecStart=/usr/sbin/ntpd -u ntp:ntp $OPTIONS present":
      ensure => present,
      path   => $file,
      line   => 'ExecStart=/usr/sbin/ntpd -u ntp:ntp $OPTIONS',
      require => Package[$time_server]
